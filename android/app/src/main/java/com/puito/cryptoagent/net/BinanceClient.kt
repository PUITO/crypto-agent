...
class BinanceClient(
    ...
    /**
     * 从 Binance 数据服务按「时刻」取价。
     * GET {base}/api/v3/klines?symbol=XXX&interval=1m&startTime={对齐到分钟}&limit=1
     * - 开仓 preferClose=false → 该分钟 open
     * - 到期 preferClose=true  → 该分钟 close（整分到期用前一分钟 close）
     */
    suspend fun priceAt(
        symbol: String,
        timeMs: Long,
