...
object StrategyEngine {
    fun signals(candles: List<Candle>, cfg: StrategyConfig): List<SignalMark> {
        ...
        for (i in candles.indices) {
            ...
            val b = side(cfg.buyRules); val s = side(cfg.sellRules)
            when {
...
object EventSim {
    fun backtest(candles: List<Candle>, signals: List<SignalMark>, symbol: String, interval: String): Pair<List<SimTrade>, Stats> {
        ...
        while (i < signals.size) {
            ...
            if (si == null) { i++; continue }
            val entryI = si + 1
            val exitI = entryI
            if (entryI >= candles.size || exitI >= candles.size) { i++; continue }
            val entry = candles[entryI]; val exit = candles[exitI]
            val long = sig.side == "B"
            val pnl = if (long) (exit.close - entry.open) / entry.open * 100
            else (entry.open - exit.close) / entry.open * 100
            trades.add(
                SimTrade(UUID.randomUUID().toString(), symbol, interval, sig.side,
